...
const i18nService = {
  ...
  switchLanguage(): void {
    const targetLanguage = this.getCurrentLanguage() === 'ru' ? 'en' : 'ru';
    this.changeLanguage(targetLanguage);
  },
  getCurrentLanguage(): string {
    return [...this.registeredInstances.values()][0]?.language ?? 'ru';
  },
  ...
  changeLanguage(language: string): void {
    this.registeredInstances.forEach((i18nInstance: i18n): void => {
